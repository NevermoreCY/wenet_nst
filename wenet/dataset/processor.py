...
def url_opener(data):
    ...
    for sample in data:
        ...
        #logging.warning(' opening url {}'.format(url))
        try:
            pr = urlparse(url)
            # local file
...
def tar_file_and_group(data):
    ...
    for sample in data:
        ...
        #print("tar_file_group stream  " , sample['stream'])
        #print("tar_file_group src ", sample['src'])
        ...
        for tarinfo in stream:
            name = tarinfo.name
            # sometimes the first line of the tarinfo is blank, so we conitnue if that case happen
            if name == "":
                continue
            pos = name.rfind('.')
            assert pos > 0
