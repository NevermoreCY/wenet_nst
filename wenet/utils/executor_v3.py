...
class Executor:
    ...
    def train(self, model, optimizer, scheduler, data_loader_aishell,data_loader_aishell2, data_loader_wenetspeech, device, writer,
        ...
        if fuse_batch:
            ...
            ratio_wenet = float(args["ratio_wenet"])
            ratio_aishell1 = float(args["ratio_aishell1"])
            ratio_aishell2 = float(args["ratio_aishell2"])

            print("************** not using batch fusion, pseudo_ratios are:"," wenet ratio = " , ratio_wenet,
                  " aishell1 ratio = " , ratio_aishell1, " aishell2 ratio = ", ratio_aishell2 , "********************")

            if (ratio_wenet + ratio_aishell1 + ratio_aishell2) != 1:
                print("************************The sum of all ration is not 1, exit!!!!****************************")
                exit()
            ...
            with model_context():
                ...
                iter_aishell2 = iter(data_loader_aishell2)
                batch_idx = 0
                dl_weight = [ratio_wenet, ratio_aishell1,ratio_aishell2]
                #rng = random.Random
                while True:

                    #print("batch id is ",batch_idx , "Time is ", time.time())
                    r = random.choices(population=[0,1,2], weights=dl_weight, k=1)[0]
                    # print(now is random)
                    if r == 0:
                        dl = iter_wenet
                        name = "wenet_speech"
                    ...
                    batch_idx += 1
                    # print("batch_idx = ", batch_idx, " name = ", name)
